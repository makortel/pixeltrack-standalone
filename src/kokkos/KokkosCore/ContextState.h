...
namespace cms {
  namespace kokkos {
    template <typename T> class ScopedContextAcquire;
    template <typename T> class SCopedContextProduce;
    ...
    private:
      ...
      void set(std::unique_ptr<impl::ExecSpaceSpecific<ExecSpace>> specific) {
        specific_ = std::move(specific);
      }

      std::unique_ptr<impl::ExecSpaceSpecific<ExecSpace>> release() {
        return std::move(specific_);
      }
